...
% figure is drawn
tmpfilename = 'figcomm_comp_targets';
...
% Plot
% without viruses
tmppos= [0.1 0.2 0.7 0.35];
tmpa2 = axes('position',tmppos);
bar(1:238, stats_top.Ntot_nov(tmpi,:), 0.5, 'stack');
ylim([0 45]);
xlim([0 239]);
...
tmpt=text(10,39,'without viruses');
set(tmpt,'interpreter','latex','fontsize',18);

% with viruses
tmppos= [0.1 0.55 0.7 0.35];
...
tmppos= [0.85 0.2 0.1 0.35];
tmpa1 = axes('position',tmppos);
tmpbh=bar(1:2,repmat(mean(stats_top.Ntot_nov),2,1),0.5,'stack');
xlim([0.5 1.5]);
tmpb=get(tmpbh(1),'BaseLine');
set(tmpb,'Visible','off');
axis off
...
% Add labels
tmp_avg = mean(stats_top.Ntot_nov);
tmp_cumsum = cumsum(tmp_avg);
tmp_cumsum = [0 tmp_cumsum];
tmp_cummid = (tmp_cumsum(1:end-1)+tmp_cumsum(2:end))/2;
for i=1:4,
