...
% figure is drawn
tmpfilename = 'figflux_cturnover_contours_targets_labels';
...
load ss_flux_target
% 
%  All of this data is consolidated here:
load ss_flux_tau_target.mat

% Plot the targets
% Here, I have inverted the tau-s (which are the residence times)
% to get turnover rates, in units of day^-1 
x=1./stats_top.tdown_nov_C(:,2);
y=1./stats_top.tdown_wv(:,2);
tmph=plot(x,y,'ko');
...
% x=log10((flux.intop_novE(tmpvalid)+flux.intop_novC(tmpvalid))/2)';
x=1./flux.tdown_nov_C(tmpvalid,2);
y=1./flux.tdown_wv(tmpvalid,2);
mYX = [y,x];
nxbins=25;
nybins=25;
minx = min(x);
...
title('Cyanobacteria turnover rate (d$^{-1}$)','fontsize',20,'interpreter','latex','horizontalalignment','center');
...
% x=(stats_top.flux.intop_novE+stats_top.flux.intop_novC)/2;
x=1./stats_top.tdown_nov_C(:,2);
y=1./stats_top.tdown_wv(:,2);
tmph=plot(x,y,'ko');
set(tmph,'markersize',4,'markerfacecolor',[0.3 0.3 0.3]);
