...
% figure is drawn
tmpfilename = 'figflux_primprod_contours_targets_labels';
...
x=(stats_top.flux.intop_novC);
y=stats_top.flux.intop;
tmph=loglog(x,y,'ko');
...
x=log10((flux.intop_novE(tmpvalid)))';
y=log10(flux.intop(tmpvalid))';
mYX = [y,x];
nxbins=25;
nybins=25;
minx = min(x);
...
title('Primary productivity','fontsize',20,'interpreter','latex','horizontalalignment','center');
