...
% figure is drawn
tmpfilename = 'figflux_oreturn_contours_targets_labels';
...
% Plot the targets
% 
%x=(stats_top.flux.o_return_novE+stats_top.flux.o_return_novC)/2;
x=(stats_top.flux.o_return_novC);
y=stats_top.flux.o_return;
tmph=loglog(x,y,'ko');
...
% x=log10((flux.o_return_novE(tmpvalid)+flux.o_return_novC(tmpvalid))/2)';
x=log10((flux.o_return_novC(tmpvalid)))';
y=log10(flux.o_return(tmpvalid))';
mYX = [y,x];
nxbins=25;
nybins=25;
minx = min(x);
...
title('DON regeneration','fontsize',20,'interpreter','latex','horizontalalignment','center');
