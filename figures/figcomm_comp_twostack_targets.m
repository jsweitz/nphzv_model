...
% figure is drawn
tmpfilename = 'figcomm_comp_twostack_targets';
...
set(gcf,'Position',[623    57   743   713]);
...
% Now sort
stats_top.Ntot_nov_C_sum=(sum(stats_top.Ntot_nov_C'))'; 
stats_top.Ntot_sum=(sum(stats_top.Ntot'))'; 
[tmpv tmpi]=sort(stats_top.Ntot_sum);

% Do some KS testing
i=1;
tmpx = stats_top.Ntot(:,i)./stats_top.Ntot_sum;
tmpy = stats_top.Ntot_nov_C(:,i)./stats_top.Ntot_nov_C_sum;
[stats_top.kstest.hH stats_top.kstest.pH] = kstest2(tmpx,tmpy,0.05,'unequal');
i=2;
tmpx = (stats_top.Ntot(:,i)+stats_top.Ntot(:,i+1))./stats_top.Ntot_sum;
tmpy = stats_top.Ntot_nov_C(:,i)./stats_top.Ntot_nov_C_sum;
[stats_top.kstest.hC stats_top.kstest.pC] = kstest2(tmpx,tmpy,0.05,'unequal');
i=4;
tmpx = stats_top.Ntot(:,i)./stats_top.Ntot_sum;
tmpy = stats_top.Ntot_nov_C(:,i)./stats_top.Ntot_nov_C_sum;
[stats_top.kstest.hZ stats_top.kstest.pZ] = kstest2(tmpx,tmpy,0.05,'unequal');
i=8;
tmpx = stats_top.Ntot(:,i)./stats_top.Ntot_sum;
tmpy = stats_top.Ntot_nov_C(:,i)./stats_top.Ntot_nov_C_sum;
[stats_top.kstest.hXon stats_top.kstest.pXon] = kstest2(tmpx,tmpy,0.05,'unequal');
i=9;
tmpx = stats_top.Ntot(:,i)./stats_top.Ntot_sum;
tmpy = stats_top.Ntot_nov_C(:,i)./stats_top.Ntot_nov_C_sum;
[stats_top.kstest.hXin stats_top.kstest.pXin] = kstest2(tmpx,tmpy,0.05,'unequal');

% Plot
% without viruses, E
%tmppos= [0.1 0.1 0.7 0.25];
%tmpa2 = axes('position',tmppos);
%bar(1:stats_top.num, stats_top.Ntot_nov_E(tmpi,:), 0.5, 'stack');
%ylim([0 45]);
%xlim([0 239]);
%set(gca,'ytick',[0 5 10 15 20 30 40]);
%set(gca,'xtick',[0 50 100 150 200 stats_top.num]);
%set(gca,'fontsize',20);
...
% without viruses, C
tmppos= [0.1 0.15 0.7 0.35];
tmpa2 = axes('position',tmppos);
bar(1:stats_top.num, stats_top.Ntot_nov_C(tmpi,:), 0.5, 'stack');
ylim([0 20]);
xlim([0 239]);
set(gca,'ytick',[0 5 10 15 20 30 40]);
set(gca,'xtick',[0 50 100 150 200 stats_top.num]);
...
% xlabel('Parameter set','fontsize',20,'verticalalignment','top','interpreter','latex');
set(gca,'fontsize',20);
xlabel('Model replicate','fontsize',20,'verticalalignment','top','interpreter','latex');
set(gca,'xtick',[1 50 100 150 200 stats_top.num]);
ylabel('N pool size, $\mu$M','fontsize',20,'verticalalignment','bottom','interpreter','latex');
tmpt=text(10,39/45*20,'without viruses, $C^{\ast}>0$');
set(tmpt,'interpreter','latex','fontsize',18);

% with viruses
tmppos= [0.1 0.5 0.7 0.35];
tmpa1 = axes('position',tmppos);
bar(1:stats_top.num, stats_top.Ntot(tmpi,:), 0.5, 'stack');
ylim([0 20]);
xlim([0 239]);
set(gca,'ytick',[0 5 10 15 20 30 40]);
set(gca,'xtick',[0 50 100 150 200 stats_top.num]);
...
axis off
for i=[1:4 8 9]
  tmpt=text(1.3,tmp_cummid(i)-0.2*(i==1),tmpsnames(i));
  set(tmpt,'fontsize',16,'interpreter','latex');
  tmpt=text(1.7,tmp_cummid(i)-0.2*(i==1),sprintf('(%4.2f)',tmp_avg(i)));
  set(tmpt,'fontsize',16,'interpreter','latex');
end
i=6;
tmpt=text(1.3,tmp_cummid(i),'$V$');
set(tmpt,'fontsize',16,'interpreter','latex');
tmpt=text(1.7,tmp_cummid(i),sprintf('(%4.2f)',sum(tmp_avg(5:7))));
set(tmpt,'fontsize',16,'interpreter','latex');
%for i=8:9,
...
tmp_cummid = (tmp_cumsum(1:end-1)+tmp_cumsum(2:end))/2;
for i=[1 2 4],
  tmpt=text(1.3,tmp_cummid(i),tmpsnames(i));
  set(tmpt,'fontsize',16,'interpreter','latex');
  tmpt=text(1.7,tmp_cummid(i),sprintf('(%4.2f)',tmp_avg(i)));
  set(tmpt,'fontsize',16,'interpreter','latex');
end
%for i=8:9,
%  tmpt=text(1.3,tmp_cumsum(i)+0.2*(i==9),tmpsnames(i));
%  set(tmpt,'fontsize',16,'interpreter','latex');
%  tmpt=text(1.7,tmp_cumsum(i)+0.2*(i==9),sprintf('(%4.2f)',tmp_avg(i)));
%  set(tmpt,'fontsize',16,'interpreter','latex');
%end
%
i=9; 
tmpt=text(1.3,tmp_cumsum(i)+0.2,tmpsnames(i));
set(tmpt,'fontsize',16,'interpreter','latex');
tmpt=text(1.7,tmp_cumsum(i)+0.2,sprintf('(%4.2f)',tmp_avg(i)));
set(tmpt,'fontsize',16,'interpreter','latex');
i=8;
tmpt=text(1.3,tmp_cummid(i),tmpsnames(i));
set(tmpt,'fontsize',16,'interpreter','latex');
